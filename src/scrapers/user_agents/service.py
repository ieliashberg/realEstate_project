...
from src.config.settings import ZILLOW_HEADERS
...
class UserAgentService:
    ...
    def get_all_user_agents(self, count: int = 50) -> List[str]:
        """
        Get all user agents from the database regardless of status.
        Used for testing all agents.
        """
        all_uas = self.session.query(UserAgent).limit(count).all()
        return [ua.user_agent for ua in all_uas]
    ...
    def test_user_agent(self, user_agent: str, test_url: str = None) -> bool:
        ...
        try:
            headers = {**ZILLOW_HEADERS, "User-Agent": user_agent}
            print(headers)
    ...
    def test_all_user_agents(self, limit: int = 50):
        """
        Test all user agents regardless of their current status.
        This will retest working, failing, and unknown user agents.
        """
        all_uas = self.session.query(UserAgent).limit(limit).all()
        
        working_count = 0
        failing_count = 0
        
        for ua in all_uas:
            print(f"Testing user agent: {ua.user_agent[:60]}... (Status: {ua.status})")
            is_working = self.test_user_agent(ua.user_agent)
            self.update_user_agent_status(ua.user_agent, is_working)
            
            if is_working:
                working_count += 1
                print(f"  ✅ SUCCESS - Status updated to working")
            else:
                failing_count += 1
                print(f"  ❌ FAILED - Status updated to failing")
            
            print()
        
        print(f"Test Summary: {working_count} working, {failing_count} failing")
        return working_count, failing_count
